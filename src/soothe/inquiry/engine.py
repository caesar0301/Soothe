...
def build_inquiry_engine(
    ...
    *,
    ...
    def generate_queries_node(state: dict[str, Any]) -> dict[str, Any]:
        ...
        sq_text = "\n".join(
            f"- {sq.get('question', sq)}" if isinstance(sq, dict) else f"- {sq}" for sq in sub_questions
        )
    ...
    def route_to_gather(state: dict[str, Any]) -> list[Send]:
        ...
        for q in queries:
            ...
            domain_hint = q.get("domain_hint", state.get("domain", "auto")) if isinstance(q, dict) else "auto"
            sends.append(
                Send(
                    "gather",
                    {
