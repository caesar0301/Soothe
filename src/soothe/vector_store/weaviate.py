    ...
    async def create_collection(self, vector_size: int, distance: str = "cosine") -> None:
        ...
        await client.collections.create(
            ...
            vector_index_config=wc.Configure.VectorIndex.hnsw(
                distance_metric=getattr(wc.VectorDistances, dist.upper().replace("-", "_"), wc.VectorDistances.COSINE),
            ),
            properties=[
    ...
    async def search(
        ...
        if filters:
            ...
            for k, v in filters.items():
                conditions.append(Filter.by_property("payload_json").contains_any([json.dumps({k: v})]))
            if len(conditions) == 1:
