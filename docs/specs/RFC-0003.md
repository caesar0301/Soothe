...
| `soothe daemon start --foreground` | Start daemon in foreground (don't daemonize). |
| `soothe daemon stop` | Stop the running daemon (SIGTERM). |
| `soothe daemon status` | Show daemon status (PID if running). |
