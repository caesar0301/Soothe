...
- **ConversationPanel** — Left column (3fr); scrollable chat history with rich text rendering.
- **PlanPanel** — Right column top (2fr height); hierarchical plan with status markers (`[ ]` pending, `[>]` in_progress, `[+]` completed, `[x]` failed).
- **ActivityPanel** — Right column bottom (3fr height); append-only scrollable log with configurable `max_lines` (default 300). Shows tool calls, subagent events, and protocol activity with type-specific rendering.
...
| Protocol event | `{event_type}: {summary}` |
