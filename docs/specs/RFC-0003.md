...
| `custom` | non-empty | `dict` | Subagent progress (e.g. `research_web_search`, `browser_step`) |
...
| `soothe init` | Initialize `$SOOTHE_HOME` (config, sessions, logs). |
...
The Textual TUI (`SootheApp`) uses a three-row layout:

```
+------------------------------+----------------------------+
|   Conversation Panel (3fr)   |  Plan Panel (2fr height)   |
|  [scrollable chat history    |  [>] Step 1: Analyze...    |
|   with markdown rendering]   |  [ ] Step 2: Design...     |
|                              +----------------------------+
|                              |  Activity Panel (3fr height)|
|                              |  . Context: 5 entries       |
|                              |  . Calling read_file        |
|                              |  [research] web search:     |
|                              |    "quantum computing"      |
|                              |  [browser] Step 3: click    |
|                              |    @ https://example.com    |
+------------------------------+----------------------------+
...
- **ConversationPanel** — Left column (3fr); scrollable chat history for **final main-assistant responses only** (plus user turns).
- **PlanPanel** — Right column top (2fr height); hierarchical plan with status markers (`[ ]` pending, `[>]` in_progress, `[+]` completed, `[x]` failed).
- **ActivityPanel** — Right column bottom (3fr height); append-only scrollable log with configurable `max_lines` (default 300). Shows protocol events, tool activity, subagent custom events, and non-conversation stream activity with type-specific rendering.
- **InfoBar** — Single row spanning full width; thread ID, event count, state, and compact subagent status.
- **ChatInput** — Bottom input with UP/DOWN arrow key navigation for input history (reversed chronological order).
...
- ActivityPanel is the progress surface:
  - Receives protocol `soothe.*` events, tool call/result lines, subagent custom events, and subagent text summaries.
  - Applies `progress_verbosity` filtering (`minimal`, `normal`, `detailed`, `debug`) for both TUI and headless text output.
  - Browser progress is surfaced via structured `browser_step` events like other subagents.

### Activity Panel Event Rendering

Events are rendered with type-specific formatting:

| Event | Display Format |
|-------|---------------|
| Tool call | `. Calling {tool_name}` (blue) |
| Tool result | `> {tool_name}  {brief content}` (green) |
| Browser step | `[browser] Step N: {action} @ {url}` |
| Subagent text | `[subagent] Text: {brief summary}` |
| Research event | `[research] {label}: {query}` |
| Claude tool use | `[claude] Tool: {tool_name}` |
| Skillify event | `[skillify] {label}: {skill}` |
| Weaver event | `[weaver] {label}: {agent_name}` |
| Protocol event | `{event_type}: {summary}` (policy includes `profile`) |

Verbose events are automatically truncated to ~80 chars for display. Full details are written to session logs and the application log.
...
Numeric prefix routing (e.g., `4 search quantum papers` to route to Research) is defined in `commands.py` but is optional and currently serves as explicit override. The function `parse_subagent_prefix_from_input()` is available but not invoked in the main flow.
...
| `$SOOTHE_HOME/sessions/{thread_id}.jsonl` | Session logs (events, tool calls, conversation) |
| `$SOOTHE_HOME/history.json` | Input history |

### Session Logging

`SessionLogger` captures:
- **`kind: "event"`** — `soothe.*` custom events (protocol activity)
...
In addition to session JSONL records, all surfaced ActivityPanel and ConversationPanel lines are persisted in `soothe.log` as structured runtime logs for auditability.

For browser automation, raw third-party `browser-use` stdout/stderr noise is suppressed from Soothe CLI/TUI output. Progress visibility is preserved through structured subagent events and filtered activity summaries.
