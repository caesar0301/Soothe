...
1. **Naming confusion** — `KeywordContext` and `StoreBackedMemory` names don't clearly communicate their role to users configuring the system.
...
- **Persistence**: Via `PersistStore` (JSON or RocksDB), one key per thread
...
#### StoreBackedMemory (default)

**Config**: `memory_backend: store`
...
- **Persistence**: Via `PersistStore`, items stored individually with a manifest
...
memory_backend: store        # "store" | "vector" | "none"
...
| Session logs | `$SOOTHE_HOME/sessions/` |
...
The current names reflect the technical implementation approach:
...
| `StoreBackedMemory` | Keyword + PersistStore | `memory_backend: store` |
| `VectorMemory` | Embedding + VectorStore | `memory_backend: vector` |

The config values (`keyword`, `vector`, `store`) are the user-facing names. Class names are internal implementation details.
