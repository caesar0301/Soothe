...
| Name | Backend | Persistence | Description |
|------|---------|-------------|-------------|
| `StoreBackedMemory` | Keyword matching | JSON or RocksDB via `PersistStore` | Default for `memory_backend=store`. Loads all items at init. |
...
```yaml
memory_backend: store     # "store" | "vector" | "none" (default: "store")
...
├── sessions/         # Session JSONL logs (one per thread)
