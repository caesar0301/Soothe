...
async def test_client_subscription_after_thread_creation(tmp_path: Path) -> None:
    ...
    try:
        ...
        await client.subscribe_thread(thread_id)
        confirmation = await await_event_type(client.read_event, "subscription_confirmed", timeout=3.0)
...
async def test_rapid_client_connections(tmp_path: Path) -> None:
    ...
    try:
        ...
        response = await await_event_type(test_client.read_event, "thread_list_response", timeout=3.0)
