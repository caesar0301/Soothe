...
def build_daemon_config(
    ...
    """Build an isolated daemon config with flexible transport configuration.

    Args:
        tmp_path: Temporary path for test isolation
        unix_socket_path: Path for Unix socket transport
        websocket_port: Optional WebSocket port (disables if None)
        http_port: Optional HTTP REST port (disables if None)
    ...
    if websocket_port is not None:
        daemon_config["transports"]["websocket"] = {
            "enabled": True,
            "host": "127.0.0.1",
            "port": websocket_port,
