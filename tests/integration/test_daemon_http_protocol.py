...
async def _await_user_messages(
    ...
    *,
    ...
    while True:
        ...
        if remaining <= 0:
            raise TimeoutError("Timed out waiting for thread history to include required user messages")
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_http_transport_thread_lifecycle(http_daemon: tuple[SootheDaemon, int]) -> None:
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=10.0) as client:
        ...
        assert all(item["metadata"].get("priority") == "normal" for item in filtered_payload["threads"])
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_http_transport_resume_thread(http_daemon: tuple[SootheDaemon, int]) -> None:
    """Layer A: verify resume endpoint accepts messages and returns resumed status."""
    daemon, port = http_daemon
    _ = daemon
    base_url = f"http://127.0.0.1:{port}"
    async with httpx.AsyncClient(base_url=base_url) as client:
        create_response = await client.post("/api/v1/threads", json={"metadata": {"tags": ["resume"]}})
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_http_transport_thread_history_continuation(http_daemon: tuple[SootheDaemon, int]) -> None:
...
async def test_http_transport_config_endpoints_are_contractual(tmp_path: Path) -> None:
    ...
    try:
        async with httpx.AsyncClient(base_url=f"http://127.0.0.1:{port}") as client:
            ...
            update = await client.put("/api/v1/config", json={"updates": {"logging": {"level": "info"}}})
